...
ll=load("SERC_krigs_sst_kd490.RData")

datasets=ed_search_adv(, query="carbon",minTime = "1995-01-01T00:00:00Z",
...
datasets$info[9,]
dataset_id=as.character(datasets$info[89,2])
# erdMH1chlamday
data <- griddap(dataset_id,
                longitude = c(-83.7, -79.5), 
                latitude = c(24, 26), 
                time = c("1990-01-01", "2000-01-01"), 
                fields = "all"
)
...
head(data.tab)
save(data.tab,file="precip_old.RData")
...
Ks=dll %>% 
  group_by(lon,lat) %>%
  summarize(Chl_med=median(precip,na.rm=T),Chl_hi=quantile(precip,0.9,na.rm=T),Chl_lo=quantile(precip,0.1,na.rm=T))
...
sst_grid=terra::extract(x = Ks.r, y = latlon.pre, method="bilinear" )[-1]
