...
ll=load("SERC_krigs_sst_prepost.RData")

datasets=ed_search_adv(, query="kd490",minTime = "1997-01-01T00:00:00Z",
                       maxTime="2019-12-31", maxLat = 26, minLat = 24, minLon = -83.7, maxLon = -79.5)
...
data <- griddap(dataset_id,
                ...
                latitude = c(24, 26), 
                time = c("2003-01-05T12:00:00Z", "2007-12-31"), 
                fields = "all"
)
data.tab=data$data
...
Ks=dll %>% 
  ...
  summarize(k490_median=median(k490,na.rm=T),k490_hi=quantile(k490,0.9,na.rm=T),k490_lo=quantile(k490,0.1,na.rm=T))
Ks$k490_range=Ks$k490_hi-Ks$k490_lo
head(Ks)
Ks1=Ks %>%
  group_by(lon,lat) %>%
  summarize(k490_median=mean(k490_median,na.rm=T),k490_range=mean(k490_range,na.rm=T))

ggplot(Ks1,aes(lon,lat,color=k490_range))+geom_point(pch=15,cex=1)+
...
data <- griddap(dataset_id,
                longitude = c(-83.7, -79.5), 
                latitude = c(24, 26), 
                time = c("2014-01-01T12:00:00Z", "2019-12-31T12:00:00Z"), 
                fields = "all"
)
data.tab=data$data
data.tab$year=sub("-.+","",data.tab$time)
...
rr=cbind(XY,sst_grid)
ggplot(rr,aes(lon,lat,color=k490_median))+geom_point(pch=15,cex=1)+
  coord_equal()+theme_bw()+scale_color_viridis()

save(rasters.pre,rasters.post,XY,file="SERC_krigs_sst_kd490_prepost.RData")
