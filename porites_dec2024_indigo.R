...
ll=load("~/Dropbox/keys_rdaforest_december2024/FL_Keys_rasters_dec2024/rasters_keys_feb2025_dl.RData")
ll=load("~/Dropbox/keys_rdaforest_december2024/FL_Keys_rasters_dec2024/Porites_env_feb2025_dl.RData")
...
# env.p$dis2land=NULL
mm=mtrySelJack(Y=IBS.p,X=cbind(env.p),nreps=30,covariates=cbind(latlon.p.utm),top.pcs=tokeep,importance.cutoff = 0.1,prop.positive.cutoff = 0.5)
mm$goodvars
# [1] "dis2land"      "SAL.B_range"   "k490_range"    "depth"         "TEMP.B_median"
# [6] "DSIGT_median" 

# [1] "depth"         "k490_median"   "SAL.B_range"   "k490_range"    "TEMP.B_median"

save(mm,gf,file=paste(pop,"_mtrySel_dl.RData",sep=""))
#load(paste(pop,"_mtrySel_dl.RData",sep=""))
...
plot_turnover(sb,rasters.post[sb$goodrows,],goodvars[1])
plot_turnover(sb,rasters.post[sb$goodrows,],goodvars[2])
plot_turnover(sb,rasters.post[sb$goodrows,],goodvars[3])
plot_turnover(sb,rasters.post[sb$goodrows,],goodvars[4])
dev.off()
...
pdf(file=paste(pop,"_bestVars_dl.pdf",sep=""),height=6,width=12)
...
pa=plot_adaptation(dirs,ras2[,bests],xy2,nclust=12,
                   ...
                   cluster.labels=T,color.scheme="011",
